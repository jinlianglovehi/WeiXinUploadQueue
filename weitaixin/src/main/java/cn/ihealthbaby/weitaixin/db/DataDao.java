					...
					for (int i = 0; i < adviceItems.size(); i++) {
						...
						try {
							db.execSQL("insert into " + DataDBHelper.tableName + " (mid,gestationalWeeks,testTime,testTimeLong,status) values (?,?,?,?,?)",
									new Object[]{String.valueOf(adviceItem.getId()), adviceItem.getGestationalWeeks(), String.valueOf(DateTimeTool.date2Str(adviceItem.getTestTime())), String.valueOf(adviceItem.getTestTimeLong()), String.valueOf(adviceItem.getStatus())});
